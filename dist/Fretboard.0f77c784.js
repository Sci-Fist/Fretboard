...
var _audioJs = require("./audio.js"); // Import audio functions
var _configJs = require("../config.js"); // Import config
 ...
 */ function setupUI() {
    ...
    Object.keys((0, _configJsDefault.default)).forEach((key)=>{
        // Convert camelCase to kebab-case for CSS variable names
        const cssVarName = `--config-${key.replace(/([A-Z])/g, '-$1').toLowerCase()}`;
        root.style.setProperty(cssVarName, (0, _configJsDefault.default)[key]);
    });
 ...
 */ function attachFretEventListeners() {
    ...
    tabDisplay.addEventListener('input', (event)=>{
        if (event.target.classList.contains('fret')) (0, _uiElementsJs.handleFretInput)(event, (0, _tabDataJs.getTabData), (0, _tabDataJs.setTabData), _renderingJs.renderTab); // Pass necessary functions
    });
    tabDisplay.addEventListener('click', (event)=>{
        if (event.target.classList.contains('fret')) (0, _uiElementsJs.showNumberCircle)(event.target);
    });
 ...
 */ function generateTabString(tabData) {
    ...
    tabData.measures.forEach((measure)=>{
        for(let stringIndex = 0; stringIndex < 6; stringIndex++){
            ...
            for(let fretIndex = 0; fretIndex < 4; fretIndex++)tabString += measure.strings[stringIndex][fretIndex] || '-';
            tabString += '|\n';
        }
 ...
 */ function showBPMInput() {
    ...
    setBPMButton.onclick = ()=>{
        ...
        if (!isNaN(newBPM) && newBPM >= 40 && newBPM <= 240) {
            (0, _tabDataJs.setTabData)({
                ...(0, _tabDataJs.getTabData)(),
                bpm: newBPM
            }); // Update BPM using setTabData
            console.log('app.js: BPM set to:', (0, _tabDataJs.getTabData)().bpm);
            // Remove the input and button after setting the BPM
            bpmInputContainer.remove();
        } else alert('Please enter a valid BPM between 40 and 240.');
    };
 ...
 */ function saveTab() {
    ...
    try {
        localStorage.setItem('tabData', JSON.stringify((0, _tabDataJs.getTabData)()));
 ...
 */ function loadTab() {
    ...
    try {
        ...
        if (savedTabData) {
            (0, _tabDataJs.setTabData)(JSON.parse(savedTabData));
            _renderingJs.renderTab((0, _tabDataJs.getTabData)());
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"jnFvT"}],"bTyvi":[function(require,module,exports,__globalThis) {
// audio.js
    ...
    try {
        ...
        playbackInterval = setInterval(()=>{
            ...
            if (currentFretIndex >= 4) {
                currentFretIndex = 0;
                currentMeasureIndex++;
                if (currentMeasureIndex >= tabData.measures.length) stopPlayback();
            }
 ...
 */ function playBeat(tabData, startTime) {
    ...
    for(let stringIndex = 0; stringIndex < 6; stringIndex++){
        ...
        if (!isNaN(fretNumber)) {
            const note = (0, _tabDataJs.getNote)(stringIndex, fretNumber, tabData.tuning);
            if (note) try {
                playNote(note, 0.2, startTime); // Play for 0.2 seconds
            } catch (error) {
                console.error(`audio.js: Error triggering note ${note}:`, error);
            }
            else console.warn(`audio.js: Could not get note for string ${stringIndex}, fret ${fretNumber}`);
        }
    }
}
/**
 ...
 */ function stopPlayback() {
    ...
    _tone.Transport.stop(); // Stop the Tone.js transport if it's running
    currentMeasureIndex = 0;
    currentFretIndex = 0;
}
/**
...
},{"./tab-data.js":"1uM0m","tone":"1Tjk2","@parcel/transformer-js/src/esmodule-helpers.js":"jnFvT"}],"1Tjk2":[function(require,module,exports,__globalThis) {
...
},{}],"kjaMg":[function(require,module,exports,__globalThis) {
// config.js
