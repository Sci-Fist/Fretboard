...
function setupUI() {
  const addMeasureButton = document.querySelector(
    ".tool-bar button:nth-child(1)",
  );
  const clearTabButton = document.querySelector(
...
function startPlayback() {
  ...
  try {
    ...
    alert(
      "Failed to initialize audio playback. Please check your browser settings.",
    );
    ...
    if (!isClickInside && !isClickOnFret) {
      ...
      setTimeout(() => {
        if (!event.target.closest(".number-circle")) {
          // Double check if click is still outside any number-circle after delay
          numberCircle.remove();
        }
